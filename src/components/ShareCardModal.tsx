...
export function ShareCardModal({
  ...
  const handleCopyText = () => {
    ...
    rankedStandings.forEach((p, idx) => {
      const crown = idx === 0 ? '🏆 [CHAMPION] ' : `${idx + 1}. `;
      const suffix = idx === 0 && isTied ? ' (Ties)' : '';
      standingsText += `${crown}${p.name} • ${p.total} pts${suffix}\n`;
    });

    const footer = `\n💬 Generated by Agnibina Sangha Bray Score Tracker`;
    const combined = title + gameInfo + standingsText + footer;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: `🔥 তাসের জনক 🔥 Winner: ${winner?.name} (${winner?.total} pts) inside Agnibina Sangha Bray.`,
        });
      } else if (navigator.share) {
        // Share text if file is not supported
        await navigator.share({
          title: '🔥 Agnibina Sangha Match Result 🔥',
          text: `🏆 Winner: ${winner?.name} compiled ${winner?.total} pts! তাসের জনক (Champion) in Agnibina Sangha. Tracked via Bray Score Tracker.`,
        });
      } else {
